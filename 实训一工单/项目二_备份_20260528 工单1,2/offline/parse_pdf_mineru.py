...
# 加载.env文件（从backend/.env读取API密钥等配置）
try:
    ...
    pass  # python-dotenv未安装时跳过，依赖系统环境变量
...
def _extract_chart_text_with_ocr(image_path: str) -> str:
    ...
    try:
        ...
        result = " ".join(texts)
        if result:
            logger.info(f"图表OCR识别成功，提取 {len(result)} 字符")
        return result
...
def block_to_text(block: dict) -> str | None:
    ...
    块类型处理策略：
      ...
      - chart → 用Vision API提取图表数据，结合caption/footnote
      - image/seal → 跳过（返回None，无可用文本）
    ...
    # 图表：用Vision API提取数据 + caption/footnote
    if block_type == "chart":
        parts = []
        # 1. 提取caption
        caption_items = content.get("chart_caption", [])
        for item in caption_items:
            t = _extract_text_recursive(item)
            if t:
                parts.append(t)
        # 2. 用Vision API读取图片
        ...
        if rel_path and _mineru_output_dir:
            ...
            vision_text = _extract_chart_text_with_ocr(abs_path)
            if vision_text:
                parts.append(vision_text)
        # 3. 提取footnote
        footnote_items = content.get("chart_footnote", [])
...
def parse_pdf_with_mineru(
    ...
    _mineru_output_dir = str(Path(output_dir) / pdf_name / parse_method)
