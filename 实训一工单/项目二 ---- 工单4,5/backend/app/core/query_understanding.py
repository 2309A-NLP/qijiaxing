...
class QueryUnderstandingService:
    ...
    def understand(self, query: str, conversation_history: Optional[List[Dict]] = None) -> QueryUnderstandingResult:
        ...
        # 【注意】指代消解后（步骤2.5）会重新计算detected_kb_name
        ...
        # 如果问题包含指代（"他们"、"该公司"、"其"等），
        # 从对话历史中提取实体名填充到查询中
        # 例如："他们募集了多少钱" → "武汉力源信息技术股份有限公司募集资金"
        if conversation_history is not None and len(conversation_history) > 0:
            ...
            logger.info(f"💬 指代消解: '{query}' → '{resolved}' (历史{len(conversation_history)}条)")
            if resolved != query:
                query = resolved
                # 重新提取实体（因为查询变了）
                entities = self._extract_entities(query)
                # 重新检测公司名（因为可能被指代消解填充了）
                detected_kb_name = resolve_kb_name(query)
                logger.info(f"🏢 消解后公司名: {detected_kb_name}")
                # 重新识别意图（因为改写后的问题意图可能变了）
                intent, confidence = self._recognize_intent(query)
    ...
    def _resolve_anaphora(self, query: str, history: List[Dict]) -> str:
        ...
        try:
            ...
            ctx = ""
            if len(history) >= 2:
                last_q = str(history[-2].get("content","")[:80])
                last_a = str(history[-1].get("content","")[:80])
                if history[-2].get("role") == "user":
                    ctx = " 最近对话主题: " + last_q + " -> " + last_a + "。"
            
            prompt = (
                "Rewrite the user latest question as a standalone query.\n"
                "Rules:\n"
                "- Replace pronouns(they/it) with actual names\n"
                "- If abbreviated(e.g. What about XXX?), use LAST exchange\n"
            ...
            import logging
            logging.getLogger(__name__).warning("_resolve_anaphora failed: %s", e)
        return query
        return query
    ...
    def _check_clarification(
        ...
        # 【作用】条件2b：检测超出招股说明书范围的问题
        # 【设计意图】招股说明书包含的是公司公开披露信息，不包含：
        #   - 个人学历/教育背景（只有工作经历和任职经历）
        #   - 股价/市值/估值（招股说明书是静态文档，不包含实时市场数据）
        #   - 未来预测/趋势（招股说明书披露历史数据，不预测未来）
        #   - 竞争对手详细信息（只披露自身业务）
        #   - 个人生活信息（只披露与公司经营相关的信息）
        #
        # 【豁免规则】招股说明书会披露以下人员的学历：
        #   - 法定代表人、董事长、总经理、核心技术人员
        #   - 其他高管的学历（如有披露）
        # 所以只有当问题问的是"非核心人员"或"个人生活"时才拒答
        out_of_scope_patterns = [
            # 股价信息：招股说明书不包含实时市场数据
            (r"(股价|市值|估值|市盈率|PE|PB|市净率|涨跌|行情)", "招股说明书不包含实时股价信息，请查阅财经网站获取最新行情。"),
            # 未来预测：招股说明书只披露历史数据
            (r"(未来|预测|趋势|展望|预期|预计|明年|后年|展望)", "招股说明书披露历史数据，不包含未来预测。"),
            # 竞争对手：招股说明书主要披露发行人自身信息
            (r"(竞争对手|同行|对标|行业排名|市场份额排名)", "招股说明书主要披露发行人自身信息，不包含竞争对手详细信息。"),
            # 个人生活信息：与公司经营无关
            (r"(婚姻|家庭|子女|年龄|生日|星座|籍贯|出生地)", "招股说明书只披露与公司经营相关的高管信息，不包含个人生活信息。"),
        ]
        for pattern, message in out_of_scope_patterns:
            if re.search(pattern, query, re.IGNORECASE):
                return True, f"抱歉，{message}"
