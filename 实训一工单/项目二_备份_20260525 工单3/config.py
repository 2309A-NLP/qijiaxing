...
def setup_logger(name: str) -> logging.Logger:
    ...
    设计要点：
      - propagate=False 防止日志重复输出到父日志器
      - 重复调用时检查handlers，避免重复添加
      - 同时输出到控制台（实时）和文件（持久）

    面试官可能问：
      Q: 为什么要同时输出到控制台和文件？
      A: 控制台输出方便实时查看运行状态，文件输出方便事后追溯。
         离线管道可能跑很长时间，如果只是打印到控制台，会话关闭后
         日志就丢了。

      Q: propagate=False 有什么用？
      A: 如果不设置，子日志器的日志会自动传给 root 日志器。
         root 日志器默认也会输出到控制台，导致每条日志打印两次。
         propagate=False 切断传播，每个模块的日志只走自己的处理器。

      Q: 为什么日志文件名要加时间戳？
      A: 每次运行生成一个独立的日志文件，方便按运行记录回溯。
         如果所有日志都写到一个文件，多次运行的内容混在一起，
         找问题时分不清是哪次运行的。

      Q: 为什么要检查 handlers 避免重复添加？
      A: setup_logger可能被多次调用（如模块重载），每次调用都
         添加新的处理器。不检查的话，日志会重复输出多次。
         Python的日志器没有内置去重机制，需要自己处理。
    """
    # 获取或创建指定名称的日志器
