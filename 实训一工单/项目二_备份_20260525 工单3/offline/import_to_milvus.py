...
def connect_milvus() -> bool:
    ...
    面试官可能问：
      Q: Milvus的gRPC协议是什么？为什么用gRPC不是HTTP？
      A: gRPC基于HTTP/2，支持双向流、头部压缩，适合高频向量检索。
         向量检索延迟敏感（ms级），gRPC比RESTful HTTP快。
         Milvus 2.x默认使用gRPC，不需要手动切换。

      Q: 连接失败后没有重试机制，为什么？
      A: 离线管道的设计原则是"一次性运行，失败就停"。如果连接失败，
         让用户去检查Milvus状态比自动重试更透明。在线服务的retry
         在retrieval.py里做，那里才是需要高可用的地方。
    """
...
def create_collection() -> Collection:
    ...
    面试官可能问：
      Q: 为什么字段定义后不可修改？
      A: Milvus的集合定义决定了存储结构和索引方式。修改字段类型/维度
         会导致底层数据文件和索引失效，所以设计为不可变。
         如果需要改字段，只能drop重建——这也是为什么有重建flag。

      Q: text字段为什么用VARCHAR不是TEXT类型？
      A: Milvus 2.x的VARCHAR支持标量过滤和索引，TEXT不支持。
         虽然我们的检索按向量相似度打分，但content_type过滤
         （只搜"text"或"table"）就是标量过滤。用VARCHAR更灵活。

      Q: 为什么embedding是1024维？
      A: BGE-M3模型的输出维度就是1024。不同模型的输出维度不同
         （如BGE-small-384维, BGE-large-1024维），必须与模型一致。
         如果换成其他模型，需要重建集合并改dim参数。
    """
    # 定义字段列表
    fields = [
        # -----------------------------------------------------------------
...
def create_index(collection):
    ...
    面试官可能问：
      Q: 为什么用IVF_FLAT不用HNSW？
      A: IVF_FLAT是倒排文件索引，适合百万级数据，召回率高。
         HNSW是图索引，速度更快但内存占用更大。
         项目数据量在万级，IVF_FLAT已经够用，不需要HNSW的额外内存开销。

      Q: nlist=128怎么确定的？
      A: nlist是聚类中心数，经验值是 sqrt(N)（N为文档总数）。
         项目约1万条数据，sqrt(10000)=100，取128。
         如果数据量增长到10万，nlist应该调整到300左右。

      Q: IP（内积）和余弦相似度什么关系？
      A: 当向量已归一化（L2范数为1）时，内积 = 余弦相似度。
         BGE-M3输出的向量默认已归一化，所以用IP度量。
         如果向量未归一化，用IP会受向量长度影响，应该用余弦相似度。
    """
    # 索引参数
    params = {
...
def load_data(collection):
    ...
    面试官可能问：
      Q: 为什么要把向量和文本解耦存储（分开两个JSONL文件）？
      A: 向量（embedding_JSONL）是float数组，体积大但不需要给人看；
         文本（chunk_JSONL）是字符串，体积小但需要给LLM阅读。
         解耦后可以分别处理：向量文件只做检索，文本文件只做展示。
         而且如果只更新文本不更新向量（如修正错别字），不需要重新跑embedding。

      Q: 为什么要分批导入（batch_size）？
      A: Milvus insert的一次调用不能传太多数据，否则内存暴涨、网络超时。
         分批导入（每批100条）把大任务拆成小任务，每批独立提交。
         如果某批失败，只会丢失那批的数据，不影响已导入的部分。

      Q: JSONL行式转Milvus列式的过程中，维度校验怎么做？
      A: 每个embedding字段必须是长度为1024的list[float]。
         代码中检查len(emb) == EMBEDDING_DIM，不一致则记录警告并跳过该条。
         这种"一个异常不影响整体"的容错思路贯穿整个离线管道。
    """
    # -------------------------------------------------------------------------
    # 步骤1：幂等检查
    # -------------------------------------------------------------------------
