...
class RetrievalService:
    ...
    def _check_cache(self, k: str) -> Optional[Dict]:
        ...
        try:
            ...
            # 【作用】如果取到了结果（不为空/None）
            if c:
                # 【作用】如果缓存的值是字典，就尝试取里面的 "result" 字段；否则直接返回原始值
                # 【原理】早期版本可能存的是原始字典，后来改成了嵌套结构，这里做兼容处理
                return c.get("result") if isinstance(c, dict) else c
        except Exception:
    ...
    async def _run_retrieval_pipeline(self, question: str, top_k: int, content_type: Optional[str], history: Optional[List[Dict]]) -> tuple:
        """执行检索管道：Query理解→编码→向量检索→混合检索→重排序

        返回：(检索结果列表, 有效问题, 有效历史, 澄清文本或None)

        管道流程：
          1. Query理解：规范化问题，处理消歧，生成澄清提示
          2. Embedding编码：将问题转为1024维向量
          3. 向量检索：Milvus相似度搜索
          4. 混合检索（可选）：BM25 + 向量检索融合
          5. 重排序（可选）：BGE-reranker-base精排

        边界情况：
          - Query理解需要澄清：直接返回空结果+澄清文本，不走后续检索
          - Query理解报错：静默跳过，用原始问题继续
          - 混合检索报错：降级为纯向量检索
          - Reranker报错：降级为Reranker前的结果
          - 所有步骤都有try-except，单步失败不影响整体

        面试官可能问：
          Q: 为什么Query理解失败后不阻止后续检索？
          A: Query理解是"锦上添花"的功能，不是"守门员"。它做错了（如误判）
             最多让检索精度降低，但不应该让整个检索停摆。
             降级策略：Query理解挂了 → 原样检索 → 最多精度差一点。

          Q: Reranker报错后怎么降级？数据流怎么走？
          A: Reranker被try-except包裹，报错时pass，rr保持为rerank之前的
             向量检索（或混合检索）结果。所以降级路径是：
             正常：向量检索→混合检索→Reranker→LLM
             降级：向量检索→混合检索→（Reranker跳过）→LLM

          Q: 为什么Query理解的澄清不需要缓存？
          A: 需澄清时返回的是"您想了解哪个时间段的财务数据？"这类模板文本，
             没有计算结果，缓存没有意义。缓存只cache有检索结果的query。
        """
        ...
        # 【逻辑】top_k 是返回最相似的多少条结果，content_type 是可选的类型过滤（比如只搜"政策"或"新闻"）
        vr = self.milvus_client.search(embedding=emb, top_k=top_k, content_type=content_type)
    ...
    async def prepare_pipeline(self, question: str, top_k: int = settings.RETRIEVAL_TOP_K, content_type: Optional[str] = None, history: Optional[List[Dict]] = None) -> dict:
        ...
        """
        rr, q, h, clarification = await self._run_retrieval_pipeline(question, top_k, content_type, history)
    ...
    async def query(self, question: str, top_k: int = settings.RETRIEVAL_TOP_K, content_type: Optional[str] = None, history: Optional[List[Dict]] = None) -> Dict:
    ...
    async def query_stream(self, question: str, top_k: int = settings.RETRIEVAL_TOP_K, content_type: Optional[str] = None, history: Optional[List[Dict]] = None):
