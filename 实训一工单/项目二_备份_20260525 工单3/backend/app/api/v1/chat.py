...
class ChatRequest(BaseModel):
    ...
    # 【作用】对话上下文历史记录，用于多轮对话场景
    # 【原理】List[Dict[str, str]] 表示一个列表，列表里每个元素是个字典，字典的 key 和 value 都是字符串
    # 【逻辑】典型格式：[{"role": "user", "content": "你好"}, {"role": "assistant", "content": "你好！"}]
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        # 【作用】调用检索服务的 query 方法，执行完整的 RAG 流程
        # 【原理】query() 内部按顺序执行：向量编码 → 语义检索 → 拼接上下文 → 调用 LLM 生成 → 返回结果
        # 【逻辑】把所有请求参数（问题、检索数量、过滤条件、历史记录）传给 query 方法
        result = await service.query(
            question=request.question,          # 用户的问题文本
            top_k=request.top_k,                # 检索返回的文档数量
            content_type=request.content_type,  # 内容类型过滤（可选）
            history=request.history             # 对话历史（可选）
        )
        # 【作用】把检索服务返回的原始字典转换成 Pydantic 模型，返回给前端
        # 【原理】ChatResponse(...) 创建响应对象，列表推导式把每个 source 字典转成 Source 对象
        # 【逻辑】FastAPI 收到这个 ChatResponse 对象后，自动序列化成 JSON 返回给客户端
        return ChatResponse(
            answer=result["answer"],                                  # 大模型生成的答案文本
            sources=[Source(**source) for source in result["sources"]]  # 引用来源列表（** 解包字典）
        )
...
async def chat_stream(request: ChatRequest):
    ...
    try:
        ...
        async def generate():
            # 【作用】第一步：执行检索管道，获取 sources 和上下文（不调 LLM）
            # 【原理】prepare_pipeline 只跑检索，返回引用来源和待生成上下文
