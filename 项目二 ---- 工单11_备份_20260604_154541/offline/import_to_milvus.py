...
def create_collection() -> Collection:
    fields = [
        ...
        FieldSchema(name="parent_text", dtype=DataType.VARCHAR, max_length=20000),
...
def load_kb_data(collection, kb_name: str):
    ...
    # 加载文本映射（包含parent_text用于Parent Document Retrieval）
    text_map = {}  # chunk_id -> {"text": ..., "parent_text": ...}
    ...
    # 解析向量JSONL，组装列式数据
    cids, texts, pss, pes, secs, sps, ctypes, tidxs, kbnames, pcids, ptexts, embs = (
        [] for _ in range(12)
    )
    ...
    with open(ep, "r", encoding="utf-8") as f:
        for ln, line in enumerate(f, 1):
            ...
            try:
                ...
                cid = d["chunk_id"]
                cids.append(cid)
                # 从text_map取文本，如果chunk_text中没有则从向量文件的parent_text取
                text_info = text_map.get(cid, {})
                if isinstance(text_info, dict):
                    texts.append(text_info.get("text", ""))
                    ptexts.append(text_info.get("parent_text", ""))
                else:
                    texts.append(str(text_info))
                    ptexts.append(str(text_info))
                pss.append(d.get("page_start", 0))
                pes.append(d.get("page_end", 0))
                secs.append(d.get("section") or "")
                sps.append(d.get("source_pdf", ""))
                ctypes.append(d.get("content_type", d.get("type", "text")))
                tidxs.append(d.get("table_index", 0))
                kbnames.append(kb_name)
                pcids.append(d.get("parent_chunk_id", ""))
                # 如果向量文件中有parent_text，优先使用
                if d.get("parent_text"):
                    ptexts[-1] = d["parent_text"]
                embs.append(d.get("embedding", d.get("vector")))
