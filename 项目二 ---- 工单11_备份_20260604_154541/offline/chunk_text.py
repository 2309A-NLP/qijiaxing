...
def chunk_table(table_text: str, table_info: dict, max_chars: int = 2000) -> list[dict]:
    ...
    原理：
      ...
      - 支持Markdown格式和HTML格式的表格
    
    参数：
      table_text: Markdown格式或HTML格式的表格文本
    ...
    表格结构（HTML）：
      ...
      按<tr>标签分割行，<td>标签分割单元格
    """
    # 检测是否为HTML格式
    if "<table>" in table_text.lower():
        return _chunk_html_table(table_text, table_info, max_chars)
    ...
    for line in data_lines:
        ...
        if len(test_text) > max_chars and buf_lines:
            ...
            chunks.append({
                "chunk_id": f"table_{table_info['page_num']}_{table_info.get('table_index', 1)}_{cid:02d}",
                "text": chunk_text,
                "type": "table",  # 类型标记
...
def _chunk_html_table(html_text: str, table_info: dict, max_chars: int = 2000) -> list[dict]:
    """
    作用：处理HTML格式的表格，转换为Markdown格式后分块
    
    原理：
      - MinerU输出HTML格式表格（<table><tr><td>...</td></tr></table>）
      - 按<tr>标签分割行，<td>标签分割单元格
      - 转换为Markdown格式（用"|"分隔）
      - 如果表格不超过max_chars，直接返回一个完整chunk
    
    参数：
      html_text: HTML格式的表格文本
      table_info: 表格元数据
      max_chars: 每个chunk最大字符数
    ...
    markdown_lines = [header_line, separator]
    ...
    if len(markdown_text) <= max_chars:
        return [{
            "chunk_id": f"table_{table_info['page_num']}_{table_info.get('table_index', 1)}_00",
            "text": markdown_text,
            "type": "table",
            "page_start": table_info["page_num"],
            "page_end": table_info["page_num"],
    ...
    for line in markdown_lines[2:]:  # 跳过表头
        test_text = header_text + "\n" + "\n".join(buf_lines + [line])
        if len(test_text) > max_chars and buf_lines:
            chunk_text = header_text + "\n" + "\n".join(buf_lines)
            chunks.append({
...
def build_chunks_by_section(pages: list[dict]) -> list[dict]:
    ...
    for sec_name, sec_pages in sections:
        # 【Contextual Chunk Headers】章节标题前缀，拼到每个chunk的text前面
        # 【原理】BGE-M3编码时会把章节标题一起编码，提高检索时的章节定位精度
        # 【格式】只在章节名非空且非"未知"时添加，避免无意义噪声
        section_header = f"[章节：{sec_name}]\n" if sec_name and sec_name != "未知" else ""

        # 分离文本页、表格页和图表页
        text_pages = [p for p in sec_pages if p.get("type") == "text"]
        table_pages = [p for p in sec_pages if p.get("type") == "table"]
        chart_pages = [p for p in sec_pages if p.get("type") == "chart"]
            ...
            for para in paragraphs:
                if buf_text:
                    ...
                    chunks.append({
                        ...
                        "text": section_header + buf_text,
                        "type": "text",
                        "page_start": pr[0],
                        "page_end": pr[1],
                        "section": sec_name,
                        "source_pdf": str(text_pages[0].get("source_pdf", "")),
                        "char_count": len(section_header) + len(buf_text),
                    })
                    cid += 1
                ...
                if len(para) <= CHUNK_MAX_CHARS:
                    ...
                    for sub in split_long_paragraph(para, CHUNK_MAX_CHARS):
                        chunks.append({
                            "chunk_id": f"chunk_{cid:04d}",
                            "text": section_header + sub,
                            "type": "text",
                            "page_start": pr[0],
                            "page_end": pr[1],
                            "section": sec_name,
                            "source_pdf": str(text_pages[0].get("source_pdf", "")),
                            "char_count": len(section_header) + len(sub),
                        })
                        cid += 1
        ...
        for table_page in table_pages:
            ...
            for tc in table_chunks:
                ...
                if section_header:
                    tc["text"] = section_header + tc["text"]
                    tc["char_count"] = len(section_header) + tc.get("char_count", len(tc["text"]))
                chunks.append(tc)
                cid += 1
        ...
        # 2.3 处理图表（OCR提取的图表文字）
        # =================================================================
        # 图表的OCR文本通常较短（50-200字符），直接作为完整chunk输出
        # 保留type="chart"标记，供检索阶段区分图表内容
        for chart_page in chart_pages:
            chart_text = chart_page.get("text", "").strip()
            if not chart_text:
                continue
            chunks.append({
                "chunk_id": f"chunk_{cid:04d}",
                "text": section_header + chart_text,
                "type": "chart",
                "page_start": chart_page.get("page_num", 0),
                "page_end": chart_page.get("page_num", 0),
                "section": sec_name,
                "source_pdf": str(chart_page.get("source_pdf", "")),
                "char_count": len(section_header) + len(chart_text),
            })
            cid += 1
