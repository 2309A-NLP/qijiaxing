...
import logging
# 【作用】引入re做过滤值清洗，防止用户输入破坏Milvus过滤表达式
import re
# 【作用】引入线程锁，保护单例工厂在并发首请求下不重复创建连接
import threading
# 【作用】引入类型注解，让函数签名更清晰
...
class MilvusClientWrapper:
    ...
    def _sanitize_filter_value(value: str) -> str:
        """清洗过滤值：移除引号/反斜杠等可能破坏过滤表达式的特殊字符。

        只保留中英文、数字、空白及 . - 等安全字符，防止用户输入注入或绕过过滤。
        """
        return re.sub(r'[^\w一-鿿\s.\-]', '', str(value))
    ...
    def search(
        ...
        返回：
            检索结果列表，每个元素包含 chunk_id/text/score/kb_name
        异常：
            RuntimeError: 检索失败
        """
        try:
            ...
            # 【作用】如果指定了内容类型，添加过滤条件（清洗后再拼接）
            if content_type:
                # 【格式】content_type == "text" 或 content_type == "table"
                filters.append(f'content_type == "{self._sanitize_filter_value(content_type)}"')

            # 【作用】如果指定了章节，添加过滤条件（清洗后再拼接）
            if section:
                # 【格式】section == "财务会计信息"
                filters.append(f'section == "{self._sanitize_filter_value(section)}"')

            # 【作用】如果指定了知识库名称，添加过滤条件（清洗后再拼接）
            # 【原理】多公司场景下，按kb_name限定检索范围，避免跨公司文档混入
            if kb_name:
                filters.append(f'kb_name == "{self._sanitize_filter_value(kb_name)}"')
            ...
            results = self.client.search(
                ...
                output_fields=[             # 返回哪些字段
                    "chunk_id", "text", "kb_name", "content_type",
    ...
    def _parse_results(self, results) -> List[Dict[str, Any]]:
        ...
        for hit in results[0]:
            ...
            parsed.append({
                ...
                "text": entity.get("text"),             # 文档块的文本内容（小块）
                "kb_name": entity.get("kb_name", ""),        # v2.0: 知识库名称
                "content_type": entity.get("content_type", "text"),  # 内容类型
                "section": entity.get("section", ""),        # 章节名称
                "page_start": entity.get("page_start", 0),   # 起始页码
                "page_end": entity.get("page_end", 0),       # 结束页码
                "parent_chunk_id": entity.get("parent_chunk_id", ""),  # 父块ID
                "parent_text": entity.get("parent_text", ""),          # 父块文本
    ...
    def get_collection_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 【作用】方式2：如果方式1没拿到数据，用has_collection验证集合存在
...
def get_milvus_client() -> MilvusClientWrapper:
    ...
    原理：双检锁（double-checked locking），并发首请求下也只创建一个实例。
    """
