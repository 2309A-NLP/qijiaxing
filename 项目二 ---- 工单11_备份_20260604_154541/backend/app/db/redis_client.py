...
# 全局缓存管理器实例（单例）
# 注意：每个单例用独立锁。get_session_manager/get_query_cache 在锁内会调用
# get_redis_cache，若共用一把不可重入锁会死锁，故分开。
_cache_manager = None
