...
class Settings(BaseSettings):
    ...
    # 【作用】允许跨域访问的来源列表（逗号分隔）
    # 【安全】生产环境必须显式配置具体域名；不能用 "*" 配合 allow_credentials=True
    ...
    def cors_origin_list(self) -> list:
        """把逗号分隔的 CORS_ORIGINS 解析为列表（去空白、去空项）。"""
    ...
    # 【说明】512太小会导致多轮对话时history被截断，2048足够支持20轮对话+长答案
...
def get_settings() -> Settings:
    ...
    """
    s = Settings()
    # 【安全】非开发环境禁止使用默认 JWT 密钥（fail-fast，避免 Token 被伪造）
    if s.ENV != "development" and s.SECRET_KEY == "your-secret-key-change-in-production":
        raise RuntimeError(
            "检测到默认 SECRET_KEY：生产/测试环境必须通过环境变量 SECRET_KEY 配置非默认密钥"
        )
    return s
