...
# 【作用】导入Redis会话管理器，用于多轮对话的短期记忆
# 【原理】SessionManager把对话历史存在Redis中，前端只需传session_id
...
async def chat(request: ChatRequest, raw_request: Request):
    ...
    try:
        ...
        # 【作用】会话来源统一：Cookie > 请求体 session_id > 新生成（与 chat_stream 一致）
        session_id = raw_request.cookies.get("session_id") or request.session_id
        if not session_id:
            session_id = str(uuid.uuid4())[:8]

        # 【作用】从Redis获取历史记录
        sm = get_session_manager()
        redis_history = sm.get_history(session_id, limit=20)
        history = []
        if redis_history:
        ...
        sm.add_to_history(session_id, "user", request.question)
        sm.add_to_history(session_id, "assistant", result["answer"])

        from fastapi.responses import JSONResponse
        response_data = {
            "answer": result["answer"],
            "sources": [source.model_dump() if hasattr(source, "model_dump") else source for source in result["sources"]]
        }
        json_resp = JSONResponse(content=response_data)
        json_resp.set_cookie(key="session_id", value=session_id, httponly=True, max_age=86400, path="/")
        return json_resp
    ...
    # 【逻辑】兜底处理：任何意料之外的异常都被转换成 500 错误，避免泄露内部信息给用户
        ...
        # 【作用】返回 HTTP 500，详细异常只进日志，不回显给用户
        # 【安全】str(e) 可能含堆栈/路径/连接串等敏感信息，对外只给模糊提示
        logger.error(f"问答接口未预期异常: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def chat_stream(request: ChatRequest, raw_request: Request):
    ...
    会话管理与普通接口 chat() 保持一致：优先用 Cookie 中的 session_id，
    没有则回退到请求体的 session_id，再没有就新生成一个。
    """
    # 【作用】延迟导入 StreamingResponse，只在需要流式响应时才加载
    from fastapi.responses import StreamingResponse

    # 【作用】统一会话来源：Cookie > 请求体 > 新生成（与 chat() 一致）
    session_id = raw_request.cookies.get("session_id") or request.session_id
    if not session_id:
        session_id = str(uuid.uuid4())[:8]
    ...
    sm = get_session_manager()
    history = []
    redis_history = sm.get_history(session_id, limit=5)
    if redis_history:
        history = [{"role": h["role"], "content": h["content"]} for h in redis_history]

    async def generate():
        # 【关键】异常处理必须放在生成器内部：StreamingResponse 返回后才执行本函数，
        # 此时响应头已发出，外层 try/except 根本捕获不到这里的异常。
        try:
            ...
            # 【作用】流式结束后，保存问答到Redis
            sm.add_to_history(session_id, "user", request.question)
            sm.add_to_history(session_id, "assistant", full_answer)
            yield "data: [DONE]\n\n"
        except Exception as e:
            # 【安全+体验】内部细节只进日志；前端收到 [ERROR] 标记可展示"回答被截断"
            logger.error(f"流式生成异常: {e}", exc_info=True)
            yield "data: [ERROR]\n\n"
            yield "data: [DONE]\n\n"

    resp = StreamingResponse(generate(), media_type="text/event-stream")
    resp.set_cookie(key="session_id", value=session_id, httponly=True, max_age=86400, path="/")
    return resp
