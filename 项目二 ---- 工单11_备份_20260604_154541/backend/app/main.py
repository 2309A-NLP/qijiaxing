...
# 限流中间件：滑动窗口，按 IP 限制请求频率
app.add_middleware(RateLimitMiddleware, max_requests=100, window_seconds=60)

# CORS：从配置读取允许的来源，避免 "*" + credentials 的高危组合
app.add_middleware(
    CORSMiddleware,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
        def _preload_all():
            ...
            if svc.features.get("cache") and svc._redis_cache:
                try:
                    # 只清缓存键、保留 session:* 对话历史。
                    # 用 pipeline 批量 UNLINK（异步删除）替代逐个 DELETE，避免大量键时阻塞启动。
                    ...
                    if batch:
                        pipe.execute()
                        deleted += batch
                    logger.info(f"🧹 Redis 缓存已清空（保留对话历史），共 {deleted} 个键")
                except Exception as e:
                    logger.warning(f"⚠️ Redis 缓存清理失败: {e}")
