...
class LLMClient:
    ...
    def __init__(self):
        ...
        # 【兜底】config 未定义 LLM_PROVIDER 时默认 sglang，避免直接实例化时 AttributeError
        self.provider = getattr(settings, "LLM_PROVIDER", "sglang").lower()
    ...
    async def _generate_openai_compatible(self, messages: List[Dict], stream: bool = False) -> str:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # 【作用】429（限流）是瞬态故障，纳入指数退避重试；其他 4xx/5xx 直接抛
                status = e.response.status_code
                if status == 429 and attempt < self.max_retries - 1:
                    logger.warning(f"LLM限流 429 (尝试 {attempt + 1})，退避后重试")
                    await asyncio.sleep(2 ** attempt)
                    continue
                logger.error(f"LLM调用失败: HTTP {status}")
                raise RuntimeError(f"LLM调用失败: HTTP {status}")
            except Exception as e:
                # 【作用】其他异常（解析错误等）直接抛，不重试
...
1. 基于提供的参考信息回答问题
2. 回答要简洁、准确、专业
3. 涉及财务数据时，注意单位（万元、亿元等）
4. 表格数据要准确引用
5. 不确定的内容不要编造

多轮对话规则（最重要）：
- 用户的后续问题可能使用省略或指代（如"他们"、"该公司"、"注册资本呢"），
  必须结合对话历史理解真实意图
- 历史对话中已经确认过的公司、人物、指标，后续直接引用，不要重复追问
- 例如：第一轮聊"武汉力源"，第二轮问"他们募集了多少钱" → 直接回答武汉力源的数据
...
回答原则（最核心）：
- 参考信息中有明确事实时，直接引用回答，不要加"参考信息未明确说明"之类的保守前缀
- 只有当参考信息中完全没有相关内容时，才说"参考信息中未涉及该问题"
- 不要把参考信息中的事实拆分后自相矛盾——要么引用事实直接回答，要么承认无相关信息
- 如果参考信息从不同角度提到了相关内容，综合这些信息回答，不要因为措辞不完全吻合就否认

关键区分原则（必须严格遵守）：
- 用户问的是什么，就找什么。不能因为参考信息有"相关"内容就回答
- 例如：用户问"学历"，参考信息只有"工作经历" → 必须说"参考信息中未涉及学历信息"
- 例如：用户问"婚姻状况"，参考信息有"高管介绍" → 必须说"参考信息中未涉及婚姻状况"
- 不能用A信息（工作经历）来回答B问题（学历），即使都关于同一个人

faithfulness规则：
- 事实性声明必须来自参考信息，不得编造参考信息中没有的内容
- 不要自行归纳分类，除非参考信息本身就有这个分类
- 不要补充参考信息中没有的扩展信息
- 如果参考信息只提到部分内容，如实回答那部分即可
- 从参考信息中的显式陈述进行直接逻辑推导不算编造。例如："A位列第二，仅次于B" → 可以推断B是第一；"A占70%" → B占30%。这类基于数学/排名关系的推理是允许的
- 非常重要的推理规则：参考信息中只要给出了部分数据，就可以做简单的排名/对比/计算推断。例如参考信息只列出了3个行业的增长率，但问题问"哪个最快"——从列表中找出最大值即可，不需要参考信息明确写出"最快的是X"
- 参考信息包含了[VLM图表描述]或[OCR图表文字]标记的内容时，这些是图表OCR/VLM自动提取的结构化数据，与正文具有同等效力，可以放心使用
