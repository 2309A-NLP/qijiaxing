...
def rrf_fuse(ranked_lists: list[list[dict]], k: int = 60) -> list[dict]:
    ...
    作用：将多条检索路径的结果按排名融合为统一排序
    原理：score = Σ 1/(k + rank_i)，k=60是论文推荐常数
    优势：不需要归一化不同检索路径的分数，只依赖排名

    参数：
      ranked_lists: 多条检索路径的结果列表，每条路径是 [dict, ...]
      k: RRF常数，控制排名靠后文档的衰减速度（默认60）
    ...
    for results in ranked_lists:
        for rank, doc in enumerate(results):
            ...
            # 保留分数最高的那份结果（用于返回text等字段）
            if cid not in best_hit or doc.get("score", 0) > best_hit[cid].get("score", 0):
                best_hit[cid] = doc
...
class RetrievalService:
    ...
    def _cache_key(self, q: str, ct: Optional[str], tk: int, history: Optional[List] = None) -> str:
        # 【作用】根据查询内容生成一个唯一的缓存键（MD5 哈希值），用于在 Redis 中查找或存储缓存
        """根据查询、内容类型、top_k、检索配置签名、对话上下文 生成 MD5 缓存键
        ...
        包含 history 的效果：
          同一问题在不同对话上下文中 → 不同缓存键
          例如："主营业务是什么"（第一轮）和"他们募集了多少钱"（第二轮）
          前者回答 武汉力源，后者不应该命中前者的缓存
        """
        config_sig = settings.cache_config_signature

        # 将对话历史纳入缓存键（取最近3条用户消息）
        history_sig = ""
        if history:
            recent = [
            ...
            if recent:
                history_sig = hashlib.md5("|".join(recent).encode()).hexdigest()[:8]

        return hashlib.md5(f"{config_sig}|{history_sig}|{q}|{ct}|{tk}".encode("utf-8")).hexdigest()
    ...
    async def _run_retrieval_pipeline(self, question: str, top_k: int, content_type: Optional[str], history: Optional[List[Dict]], kb_name: Optional[str] = None) -> tuple:
        ...
        if self.features["query_understanding"] and self.query_understanding:
            try:
                ...
                if u.sub_queries and len(u.sub_queries) > 1:
                    # 【作用】多子查询分别检索 + RRF 融合（Query Decomposition 落地）
                    # 【原理】每个子查询独立编码检索，覆盖不同语义维度，最后用 RRF 排名融合
                    # 【优势】比 join 成一个字符串做单次检索更精准——不同子查询可能匹配不同 chunk
                    logger.info(f"🔍 多子查询检索: {u.sub_queries}")
                    sub_results = []
                    for sq in u.sub_queries:
                        sq_emb = self.embedding_service.encode(sq)
                        sq_hits = self.milvus_client.search(
                            embedding=sq_emb, top_k=top_k * 3,
                            content_type=content_type, kb_name=kb_name
                        )
                        sub_results.append(sq_hits)
                    # RRF 融合多路结果
                    vr = rrf_fuse(sub_results, k=60)
                    # 融合后截取 top_k*6 给后续 reranker
                    vr = vr[:top_k * 6]
                    # q 保留主查询（第一个子查询），用于后续 reranker
                    q = u.sub_queries[0]
            except Exception:
        ...
        # 【设计意图】从top_k*6降为top_k*3：经实测top_k*3足够覆盖相关文档，
        #            减少reranker候选数可节省~50% reranker时间
        vr = self.milvus_client.search(embedding=emb, top_k=top_k * 3, content_type=content_type, kb_name=kb_name)
    ...
    def _build_sources(retrieval_results: List[Dict]) -> List[Dict]:
        ...
        return [{"text": r.get("text", "")[:200] + "..." if len(r.get("text", "")) > 200 else r.get("text", ""),
                 "page": r.get("page_start", 0), "section": r.get("section", ""),
                 "content_type": r.get("content_type", ""), "score": round(r.get("score", 0), 4)}
    ...
    async def prepare_pipeline(self, question: str, top_k: int = settings.RETRIEVAL_TOP_K, content_type: Optional[str] = None, history: Optional[List[Dict]] = None, kb_name: Optional[str] = None) -> dict:
        ...
        ctx = [r.get("parent_text", r.get("text", "")) for r in rr] if rr else []
        return {
    ...
    async def query(self, question: str, top_k: int = settings.RETRIEVAL_TOP_K, content_type: Optional[str] = None, history: Optional[List[Dict]] = None, kb_name: Optional[str] = None) -> Dict:
        ...
        """
        try:
            # 【作用】根据问题、内容类型、top_k、对话历史 生成一个唯一的缓存键
            # 【逻辑】history也纳入key：同一问题在不同对话中不共享缓存
            ck = self._cache_key(question, content_type, top_k, history)
            ...
            ctx = [r.get("parent_text", r.get("text", "")) for r in rr]
